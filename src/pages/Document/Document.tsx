...
export const Document = () => {
  ...
  const { selectedOffer, step, offers } = useApplication();
  ...
  if (String(selectedId) !== applicationId || (step < 3 && step !== 4 && !(step === 0 && offers))) {
    return <Navigate to="NotFound" replace />;
  }

  return (
    <main className={styles.main + ' container'}>
      {(step === 3 || step === 0) && <PaymentForm data={data.credit.paymentSchedule} />}
